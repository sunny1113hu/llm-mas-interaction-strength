...
def _plot_t30_panel(ax, trials: list[dict]) -> None:
    baseline = _filter_trials(trials, grid_size=4, steps=10)
    t30 = _filter_trials(trials, grid_size=4, steps=30)
    mus_all: list[float] = []
    mus_all.extend(
        _plot_series(
            ax,
            _metric_by_mu(baseline, "final_dispersion"),
            color=CEC_BLUE,
            label="T=10",
            linestyle="-",
            marker="o",
    ...
    mus_all.extend(
        _plot_series(
            ax,
            _metric_by_mu(t30, "final_dispersion"),
            color=CEC_ORANGE,
            label="T=30",
            linestyle="--",
            marker="s",
    ...
    ax.set_xlabel("μ")
    ax.set_ylabel("MAD")
    ax.text(0.5, -0.25, "(a)", transform=ax.transAxes, ha="center", va="top", fontsize=9)
    ax.legend(frameon=False, loc="upper right", handlelength=2.2)


def _plot_t30_panel_diffusion(ax, trials: list[dict]) -> None:
    baseline = _filter_trials(trials, grid_size=4, steps=10)
    t30 = _filter_trials(trials, grid_size=4, steps=30)
    mus_all: list[float] = []
    mus_all.extend(
        _plot_series(
            ax,
            _metric_by_mu(baseline, "roughness_final"),
            color=CEC_BLUE,
            label="T=10",
            linestyle="-",
            marker="o",
    ...
    mus_all.extend(
        _plot_series(
            ax,
            _metric_by_mu(t30, "roughness_final"),
            color=CEC_ORANGE,
            label="T=30",
            linestyle="--",
            marker="s",
    ...
    ax.text(0.5, -0.25, "(b)", transform=ax.transAxes, ha="center", va="top", fontsize=9)
    ax.legend(frameon=False, loc="upper right", handlelength=2.2)


def main() -> None:
    parser = argparse.ArgumentParser(description="Create journal Fig8: robustness panels.")
    ...
    consensus_trials = _load_trials(args.outputs_root, CONSENSUS_T_RUNS)
    diffusion_trials = _load_trials(args.outputs_root, DIFFUSION_T_RUNS)
    ...
    _plot_t30_panel(axes[0], consensus_trials)
    _plot_t30_panel_diffusion(axes[1], diffusion_trials)
    save_figure(fig, args.out_dir / "Fig8.pdf", pad_inches=0.0, bbox_inches=None)
