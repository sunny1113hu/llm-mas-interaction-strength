...
def _filter_trials(trials: list[dict], *, grid_size: int) -> list[dict]:
    filtered = []
    for record in trials:
        ...
        if (record.get("prompt_mode") or "with_pi") == "no_pi":
            continue
        if int(record.get("grid_size", 4)) != grid_size:
            continue
...
def _plot_grid_panel(ax, trials: list[dict], *, metric_key: str, ylabel: str, panel_label: str) -> None:
    style = {
        4: (CEC_BLUE, "4x4", "-", "o"),
        6: (CEC_GREEN, "6x6", "--", "s"),
        8: (CEC_ORANGE, "8x8", "-.", "^"),
    ...
    for grid_size in [4, 6, 8]:
        subset = _filter_trials(trials, grid_size=grid_size)
        color, label, linestyle, marker = style[grid_size]
        if not subset:
            continue
        mus_all.extend(
            _plot_series(
                ax,
                _metric_by_mu(subset, metric_key),
                color=color,
                label=label,
                linestyle=linestyle,
                marker=marker,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create journal Fig9: grid robustness panels.")
    ...
    consensus_trials = _load_trials(args.outputs_root, CONSENSUS_GRID_RUNS)
    diffusion_trials = _load_trials(args.outputs_root, DIFFUSION_GRID_RUNS)
    ...
    _plot_grid_panel(axes[0], consensus_trials, metric_key="final_dispersion", ylabel="MAD", panel_label="a")
    _plot_grid_panel(axes[1], diffusion_trials, metric_key="roughness_final", ylabel="roughness", panel_label="b")
    save_figure(fig, args.out_dir / "Fig9.pdf", pad_inches=0.0, bbox_inches=None)
