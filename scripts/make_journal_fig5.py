...
LINE_STYLES = {
    "Qwen/Qwen3-8B": ("-", "o"),
    "mistralai/Ministral-3-14B-Instruct-2512": ("--", "s"),
    "microsoft/Phi-4-mini-instruct": ("-.", "^"),
}
...
def _plot_panel(
    ...
    *,
    ...
    ylabel: str,
    panel_label: str,
    diffusion_scale: bool = False,
) -> None:
    mus_all: list[float] = []
    for model_id in MODELS_SCALE:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create journal Fig5: cross-scale model-dependence panels.")
    ...
    save_figure(fig, args.out_dir / "Fig5.pdf", pad_inches=0.0, bbox_inches=None)
